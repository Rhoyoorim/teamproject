...
public class PobbangDAO {
	...
	public PobbangDAO() {
		String[] name = 	{"피카츄빵 피카피카피 촉촉 치즈케익",
				"파이리빵 파이리의 화르륵 핫소스팡",
				"꼬부기빵 꼬부기의 달콤파삭 꼬부기빵",
				"로켓단빵 돌아온 로켓단 초코롤",
				"푸린빵 푸린의 폭신폭신 딸기 크림빵",
...
//			vo.setCnt(0);
	...
	//	update()-재고설정 // boolean값으로 return -(cnt 수량 추가 후 return true / 수행실패 return false)
	...
	public ArrayList<PobbangVO> selectAll(PobbangVO vo) {
		...
		for(int i = 0; i < arr.size(); i++) {
			if(vo==null) { // vo가 null이면 재고량 0인 품목 제외하고 모든 품목 추가
				if(arr.get(i).getCnt()!=0) { // 리스트의 요소중 cnt가 0이 아닌 것만 추출하여 새로운 리스트로 생성
					datas.add(arr.get(i));
					continue;
				}
			}
	...
	public String seal(PobbangVO vo) {
		Random r=new Random();
...
//		int rand = 100;
	...
	//	public int startNum(PobbangVO vo) {
	//		int startNum=arr.get(0).getNum();
	//		return startNum;
	//	}
	//	public int endNum(PobbangVO vo) {
	//		if(arr.size()!=0) {
	//		int endNum=arr.get(arr.size()-1).getNum();
	//		return endNum;
	//		}
	//		return 0;
	//	} 
